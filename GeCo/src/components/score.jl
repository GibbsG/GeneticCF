...
function score(classifier::FICO_CLASSIFIER, counterfactuals::DataFrame, desired_class; extra_col = NUM_EXTRA_COL)::Vector{Float64}
    preds = FicoClassifier.predict(classifier, counterfactuals[!, 1:end-extra_col])
    if desired_class == 0
        return 1 .- preds
    end
    return preds
