...
function generateThreds(rules, X)
    threshs = []
...
function groundTruthExperiment(X, p, thresholds, rules)
    ...
    file_naive = "rule_based_model/scripts/results/ground_truth_credit_random.jld"
...
# rules = [[:EducationLevel, :MaxBillAmountOverLast6Months, :MaxPaymentAmountOverLast6Months, :MostRecentBillAmount], [:AgeGroup, :MostRecentBillAmount, :TotalOverdueCounts, :EducationLevel], [:AgeGroup, :MonthsWithLowSpendingOverLast6Months, :MaxPaymentAmountOverLast6Months]]
# rules = [[:EducationLevel, :MaxBillAmountOverLast6Months, :MaxPaymentAmountOverLast6Months, :MostRecentBillAmount], [:AgeGroup, :MostRecentBillAmount, :TotalOverdueCounts, :EducationLevel], [:AgeGroup, :MonthsWithLowSpendingOverLast6Months, :MaxPaymentAmountOverLast6Months, :TotalMonthsOverdue]]
# rules = [[:EducationLevel, :MaxBillAmountOverLast6Months, :MaxPaymentAmountOverLast6Months, :MostRecentBillAmount]]
# threshs = [[orig_instance[s] for s in rule] for rule in rules]

# this_classifier = @ClassifierGenerator(rules, threshs)
# rules_generated,  = generate_rules(orig_instance, X, this_classifier, initPLAF(), geco_initial = false, epslin = 0.00, max_num_population = 100, max_generation = 50, geco_mutation = false, geco_check_size = 100);
# print_rule(orig_instance, rules_generated[1])
rules = generatRules(vars, num)
# println(rules)
threds = generateThreds(rules, X)
